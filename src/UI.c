...
void displayAvailableUpgrades(SDL_Renderer *renderer, Player *player)
{
  ...
  sprintf(text, "Available upgrades : %d\n[1]speed: %d\n[2]bullet damage: %d\n[3]health: %d\n[4]reload :%d",
          player->availableUpgrades, player->speedLevel, player->bulletDamageLevel, player->healthLevel, player->reloadSpeedLevel);
