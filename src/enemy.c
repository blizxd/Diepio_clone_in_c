...
int createEnemy(SDL_Point pos)
{
  ...
  if (!checkEnemyObsCollision(lastIndex) && !checkCollisionBetweenEnemies(lastIndex) && !checkCollision(camera, tempEnemy->collider))
  {
    return 1;
  }
  return 0;
...
void moveEnemy(int enemyId, Player *player)
{
  ...
  if (
      (enemy->pos.x < SCREEN_WIDTH / 2) ||
      (enemy->pos.x + enemy->pos.w > LEVEL_WIDTH + SCREEN_WIDTH / 2) ||
      (checkEnemyObsCollision(enemyId)) ||
      (checkPlayerCollision(enemyId, player)) ||
...
void increaseMaxEnemies()
{
  ...
  if (SDL_GetTicks() - msSinceMaxEnemiesIncrease > MAX_ENEMIES_INCREASE_INTERVAL && currentMaxEnemies < MAX_ENEMIES)
  {
    currentMaxEnemies++;
