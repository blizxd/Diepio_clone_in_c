...
void renderObstacle(Obstacle *obs, SDL_Rect camera, SDL_Renderer *renderer)
{
  ...
  if (obs->hp != obs->maxHp)
    renderHealthBar(renderer, (float)obs->hp / obs->maxHp, (SDL_Point){obs->pos.x + obs->pos.w / 2, obs->pos.y}, camera);
}
...
void createRandomObstacles(SDL_Renderer *renderer, int obsCount)
{
  ...
  while (obstacleGroup.activeCount < MAX_OBSTACLES && createdObs < obsCount)
  {
    ...
    if (createObstacle(&obstacleGroup.obstacles[obstacleGroup.activeCount], renderer, type, (SDL_Point){x_pos, y_pos}))
    {
      createdObs++;
    }
  }
